from accounts.models import Company, Manager
...
class SpecialEventsPlan(models.Model):
    company = models.ForeignKey('accounts.Company', on_delete=models.CASCADE, related_name='specialevents_plans')
    ...
    created_by = models.ForeignKey('accounts.Manager', null=True, blank=True, on_delete=models.SET_NULL, related_name='specialevents_plans')

    def get_status_bootstrap(self):
        return BOOTSTRAP_SPANISH_PLAN_STATUS.get(self.status)

    def has_results(self):
        return self.drivers.exclude(results='').count() > 0
...
class DriverTask(models.Model):
    plan = models.ForeignKey(SpecialEventsPlan, on_delete=models.CASCADE, related_name='drivers')
    name = models.CharField(max_length=64)
    finished = models.BooleanField(default=False)
    results = models.FileField(upload_to='specialevents/results/', blank=True, null=True, default=None)
    ...
    ended_at = models.DateTimeField(blank=True, null=True, default=None)
    first_row = models.PositiveIntegerField()
    ...
    def __str__(self):
        return "{}| store:{}, plan:{}".format(self.id, self.name, self.plan)
